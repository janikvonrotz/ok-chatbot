...
export default (bot, message) => {
    ...
    Deals.findOne(match, (error, deal) => {
        if(error) {
            ...
            if(deal === null) {
                bot.reply(message, I18n('ctrl_delete_deal.no_longer_valid'));
            } else {
                ...
                if(user === null) {
                    ...
                    let updateUserPromise = new Promise((resolve, reject) => {
                        ...
                        if(index > -1) {
                            user.savedDeals.splice(index, 1);
                            Users.update({_id: user.id}, { $set: { savedDeals: user.savedDeals }}, (error, response) => {
                                if(error) {
                                    reject(error);
                                } else {
                                    resolve(I18n('ctrl_delete_deal.confirm_deal_deletion'));
                                }
                            });
                        } else {
                            resolve(I18n('ctrl_delete_deal.not_saved_deal'));
                        }
                    });
